...
-- expressions involving shifts, rotates, masking, etc. This package pushes those operations
-- into a non-SPARK body where they won't interfer with the proofs of code that uses them.
--
-- Eventually SPARK may be upgraded to allow direct proof of more bit manipulations. When that
-- occurs the subprograms defined here could be inline expanded into the code that uses them. To
-- check if SPARK is ready, try setting the SPARK_Mode of this package's body to "On" and seeing
-- if SPARK can prove the bodies of these subprograms.
--
